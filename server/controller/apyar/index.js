...
async function getAll(req,res){
    try {
        let apyars = await ApyarModel.find().sort({[apyar_fields.DATE]:-1})
        res.status(200).json({apyars,success:true})
    } catch (error) {
...
async function getOne(req,res){
    try {
        ...
        if(req.query.search === undefined){
            ...
            if(searchArr[0] === undefined || searchArr[1] === undefined) throw `'key' or 'value' not found or empty`;
            key = searchArr[0];
            val = searchArr[1];
...
async function updateOne(req,res){
    try {
        ...
        res.status(200).json({apyar:{...req.body,id:req.params.id},success:true})
